...
describe("Timelock", accounts => {
  ...
  it("should allow recovering tokens accidentally sent", async () => {
    ...
    await expect(
      await timelock.claim(token.address, ethers.constants.AddressZero)
    ).to.emit(timelock, "Claim").withArgs(token.address, 15, deployer.address);
  ...
  it("shouldn't let anyone set a lock", async () => {
    await expect(
      timelock.connect(other).lock(token.address, 1, ethers.constants.AddressZero)
  ...
  it("should let the owner set a lock", async () => {
    await token.transfer(timelock.address, 15);
    await expect(
      await timelock.lock(token.address, 6, ethers.constants.AddressZero)
    ).to.emit(timelock, "Lock").withArgs(token.address, 6, deployer.address);
    next = (await waffle.provider.getBlock("latest")).timestamp + MONTH;
    expect(await timelock.nextLockTime(token.address, ethers.constants.AddressZero)).to.equal(next);
    expect(await timelock.remainingMonths(token.address, ethers.constants.AddressZero)).to.equal(6);
  });
  ...
  it("shouldn't allow claiming before the lock expires", async () => {
    ...
    await expect(
      timelock.claim(token.address, ethers.constants.AddressZero)
    ).to.be.revertedWith(`Locked("${token.address}", ${time}, ${next}, ${deployer.address})`);
  });

  it("should allow claiming when the lock expires", async () => {
    let sum = 0;
    let remaining = await timelock.remainingMonths(token.address, ethers.constants.AddressZero);
    while (remaining !== 0) {
      ...
      await expect(
        await timelock.claim(token.address, ethers.constants.AddressZero)
      ).to.emit(timelock, "Claim").withArgs(token.address, amount, deployer.address);
      sum += amount;
      ...
      next += MONTH;
      expect(await timelock.nextLockTime(token.address, ethers.constants.AddressZero)).to.equal(next);
      expect(await timelock.remainingMonths(token.address, ethers.constants.AddressZero)).to.equal(remaining - 1);
      remaining = await timelock.remainingMonths(token.address, ethers.constants.AddressZero);
    }
  });
